...
class RekognitionVideo:
    ...
    def __init__(self, video, video_name, rekognition_client):
        ...
        :param kinesis_client: A Boto Kinesis client
        """
        self.video = video
...
def usage_demo():
    ...
    bucket = s3_resource.create_bucket(
        ...
        # edited: source https://stackoverflow.com/questions/51912072/invalidlocationconstraint-error-while-creating-s3-bucket-when-the-used-command-i
    ...
    video = rekognition_client.create_stream_processor(
        Input={
            'KinesisVideoStream': {
                'Arn': 'arn:aws:kinesisvideo:us-east-1:483493240507:stream/MyKVStream/1657878557071',
                # 'FrameOffsetInSeconds': 2
            }
        },
        Output={
                "KinesisDataStream": {
                    "Arn": "arn:aws:kinesis:us-east-1:483493240507:stream/words",
                    }
        },
        Name='demo-stream-processor',
        RoleArn='arn:aws:iam::483493240507:role/kinesis_role',
        Settings={
            'ConnectedHome': {
                "Labels": ["ALL"],
                "MinConfidence": 50
    ...
    name_resource_rand=  str(''.join(random.choice(letters) for i in range(10)))
    iam_resource = boto3.resource('iam')
    sns_resource = boto3.resource('sns')
    sqs_resource = boto3.resource('sqs')
    video.create_notification_channel(
        name_resource_rand, iam_resource, sns_resource, sqs_resource)
